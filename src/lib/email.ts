...
const TEMPLATES: Record<EventType, TemplateDefinition> = {
  reservation_confirmed: {
    ...
    subject: "Your spot is reserved \u2014 GrowthAgency.dev",
...
<body style="margin:0;padding:0;background-color:#f8fafc;font-family:'Inter',system-ui,-apple-system,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f8fafc;padding:40px 20px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td style="background-color:#ffffff;border-radius:16px;padding:40px 32px;">
          <h1 style="margin:0 0 24px;font-size:22px;font-weight:700;color:#0f172a;">Your spot is reserved!</h1>
          <p style="margin:0 0 16px;font-size:16px;color:#475569;line-height:1.7;">
            Thank you for reserving your spot in the <strong>{{cohortMonth}}</strong> cohort. I'll be in touch within 24 hours to schedule our 15-minute strategy call.
          </p>
          <p style="margin:0 0 16px;font-size:16px;color:#475569;line-height:1.7;">
            During that call, I'll learn about your brand and design your PR strategy. Then we get to work.
          </p>
    ...
    textBody: `Your spot is reserved!

Thank you for reserving your spot in the {{cohortMonth}} cohort. I'll be in touch within 24 hours to schedule our 15-minute strategy call.
...
Founder, GrowthAgency.dev`,
  ...
  reservation_notification: {
    ...
    subject: "New reservation: {{cohortMonth}} \u2014 {{email}}",
    htmlBody: `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;padding:16px 0;">
  <h3 style="color:#0f172a;margin-bottom:12px;">New spot reserved on growthagency.dev</h3>
  <table style="color:#334155;line-height:1.8;">
    <tr><td style="padding-right:16px;color:#94a3b8;">Cohort</td><td><strong>{{cohortMonth}}</strong></td></tr>
    <tr><td style="padding-right:16px;color:#94a3b8;">Email</td><td>{{email}}</td></tr>
  </table>
  <p style="color:#94a3b8;margin-top:12px;font-size:13px;">Payment received via Stripe. Reach out within 24h to schedule the strategy call.</p>
</div>`,
    textBody: `New spot reserved on growthagency.dev\nCohort: {{cohortMonth}}\nEmail: {{email}}\nPayment received via Stripe. Reach out within 24h to schedule the strategy call.`,
  },
  ...
  <p style="color:#64748b;line-height:1.6;margin-bottom:16px;">
    We received your interest in <strong>{{serviceName}}</strong>. I'll be in touch shortly.
  </p>
  ...
  <p style="color:#0f172a;font-weight:500;">
    ...
    textBody: `Thanks for reaching out! We received your interest in "{{serviceName}}". I'll be in touch shortly. Feel free to reply to this email with any questions. \u2014 Kevin Lourd, GrowthAgency.dev`,
  },
...
export async function sendEmail(
  ...
  if (!apiKey) {
    console.warn("[growthagency] Missing POSTMARK_API_KEY, skipping send");
    return;
  }

  const template = TEMPLATES[eventType as EventType];
  if (!template) {
    console.error(`[growthagency] Unknown event type: ${eventType}`);
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[growthagency] Postmark send failed:", res.status, text);
    } else {
      console.log(`[growthagency] Sent ${eventType} to ${recipientEmail}`);
    }
  } catch (err) {
    console.error("[growthagency] Postmark send error:", err);
  }
}
